...
import { sha256Hex } from './app/core/chunker.js';
...
import { BridgeClient } from './app/core/bridge-client.js';
...
import { SupabaseSessionStore } from './app/src/session.js';
...
console.log(`\n  Write complete: ${Date.now() - t1} ms`);

// 3. Flush to Supabase
const t2 = Date.now();
const stats = await writer.flush();
console.log(`  Flush to Supabase: ${Date.now() - t2} ms (${stats.flushed} items)`);

// 4. Drain to Telegram
const t3 = Date.now();
const drainResult = await sink.drain(sid);
console.log(`  Drain to Telegram: ${Date.now() - t3} ms (${drainResult.uploaded} chunks, ${drainResult.bytes} bytes)`);
...
const restoredChecksum = restored.checksum;

// Compute expected checksum from the full content
...
const checksumMatch = restoredChecksum === expectedChecksum;

console.log(`  Cold restore: ${coldMs} ms`);
console.log(`  Restored size: ${(restoredBytes / 1024 / 1024).toFixed(2)} MiB`);
console.log(`  Throughput: ${(restoredBytes / 1024 / 1024 / (coldMs / 1000)).toFixed(2)} MiB/s`);
...
console.log(`\n=== SUMMARY ===`);
console.log(`Write:         ${Date.now() - t1} ms`);
console.log(`Flush:         ${Date.now() - t2} ms`);
console.log(`Drain:         ${Date.now() - t3} ms`);
